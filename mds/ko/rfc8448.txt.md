...
- TLS 1.3에 대한 예제 핸드셰이크 추적

---
# **Abstract**

이 문서에는 TLS 1.3 핸드셰이크의 예가 포함되어 있습니다. 이러한 핸드셰이크를 재현할 수 있도록 개인 키와 입력이 제공됩니다. 비밀, 트래픽 키 및 IV를 포함한 중간 값이 표시되어 이러한 값에 대해 점진적으로 구현을 확인할 수 있습니다.
...
이 문서의 현재 상태, 정오표 및 피드백 제공 방법에 대한 정보는 https://www.rfc-editor.org/info/rfc8448에서 얻을 수 있습니다.
...
TLS 1.3\[TLS13\]은 새로운 키 일정과 여러 가지 새로운 암호화 작업을 정의합니다. 이 문서에는 모든 중간 값을 보여주는 샘플 핸드셰이크가 포함되어 있습니다. 이를 통해 각 암호화 계산의 입력과 출력을 독립적으로 검사하여 구현을 점진적으로 검증할 수 있습니다.

이러한 값을 가져오고 동일한 출력이 생성되는지 확인하여 구현을 확인할 수 있도록 개인 키가 추적에 포함됩니다.

참고: HMAC 기반 Extract-and-Expand Key Derivation Function\(HKDF\) \[RFC5869\]의 호출에는 레이블이 지정되지 않지만 HKDF에서 사용하는 레이블을 사용하여 식별할 수 있습니다.

---
## **2.  Private Keys**

임시 개인 키는 추적에서 생성되는 대로 표시됩니다.

대부분의 예에서 서버는 개인 키가 다음과 같은 RSA 인증서를 사용합니다.
...
지수 1: 3f 57 34 5c 27 fe 1b 68 7e 6e 76 16 27 b7 8b 1b 82 64 33 dd 76 0f a0 be a6 a6 ac f3 94 90 aa 1b 47 cd a4 86 9d 68 f5 84 dd 5b 50 29 bd 32 09 3b 82 58 66 1f e7 15 02 5e 5d 70 a4 5a 08 d3 d3 19
...
이 예에서는 가능한 가장 간단한 핸드셰이크가 완료되었습니다. 서버는 인증되지만 클라이언트는 익명으로 유지됩니다. 연결 후 몇 개의 애플리케이션 데이터 옥텟이 교환됩니다. 서버는 재개된 모든 세션에서 0-RTT 데이터 사용을 허용하는 세션 티켓을 보냅니다.

{client}는 임시 x25519 키 쌍을 생성합니다.
...
{client}는 ClientHello 핸드셰이크 메시지를 구성합니다.
...
{server}는 ServerHello 핸드셰이크 메시지를 구성합니다.
...
{server} 핸드셰이크 "tls13 파생"에 대한 비밀 파생:
...
{server} 마스터 "tls13 파생"에 대한 암호 파생:
...
{server}는 핸드셰이크 데이터에 대한 쓰기 트래픽 키를 파생합니다.
...
{server}는 EncryptedExtensions 핸드셰이크 메시지를 구성합니다.
...
{server}는 CertificateVerify 핸드셰이크 메시지를 구성합니다.
...
{server}는 애플리케이션 데이터에 대한 쓰기 트래픽 키를 파생합니다.
...
{server}는 핸드셰이크 데이터에 대한 읽기 트래픽 키를 파생합니다.
...
{client} 핸드셰이크 "tls13 파생"에 대한 비밀 파생:
...
{client}는 애플리케이션 데이터에 대한 쓰기 트래픽 키를 파생합니다.
...
{server}는 NewSessionTicket 핸드셰이크 메시지를 구성합니다.
...
이 핸드셰이크는 섹션 3의 핸드셰이크에서 다시 시작됩니다. 서버가 0-RTT를 허용하는 세션 티켓을 제공했고 클라이언트가 0-RTT에 대해 구성되었으므로 클라이언트는 0-RTT 데이터를 보낼 수 있습니다.

참고: PSK 바인더는 Finished와 동일한 구성을 사용하므로 여기에서 Finished로 레이블이 지정됩니다.
...
- 02 02 02 00 2일 00 02 01 01 00 1c 00 02 40 01 00 15 00 57 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 0 0 00 00 00 00 00 00 00 00 00 9 0 bf 1b 00 70 광고 3c 49 88 83 c9 36 7c 09 a2 be 78 5a bc 55 cd 22 60 97 a3 a9 82 11 72 83 f8 2a 03 a1 43 ef d3 ff 5d d3 6d 64 e8 61 be 7f d6 1d 28 27 db 27 9c CE 14 50 77 d4 54 a3 66 4d 4e 6d a4 d2 9e e0 37 25 a6 a4 다 fc d0 fc 67 d2 애 a7 05 29 51 3e 3d a2 67 7f a5 90 6c 5b 3f 7d 8f 92 f2 28 bd a4 0d 다 72 14 70f9 fb f2 97 b5 ae a6 17 64 6f ac 5c 03 27 2e 97 07 27 c6 21 a7 91 41 ef 5f 7d e6 50 5e 5b fb c3 88 e9 33 43 69 40 93 93 4a e4 d3 57 fa d6 aa cb

```text
   {client}  calculate PSK binder:
```

- ClientHello 접두어\(477 옥텟\): 01 00 01 fc 03 03 1b c3 ce b6 bb e3 9c ff 93 83 55 b5 a5 0a db 6d b2 1b 7a 6a f6 49 d7 b4 bc 41 9d 78 76 48 7d 95 00 00 0 6 13 01 13 03 13 02 01 00 01 cd 00 00 00 0b 00 09 00 00 06 73 65 72 76 65 72 01 00 01 00 00 0a 00 14 00 12 00 1d 00 17 00 18 00 19 01 00 01 01 01 02 01 03 01 04 00 33 00 26 00 24 00 1d 00 20 e4 ff b6 8a c0 5f 8d 96 c9 9d a2 66 98 34 6c 6b e1 64 82 바드 다페 05 1a 66 b4 f1 8d 66 8f 0b 0 0 2아 00 00 00 2b 00 03 02 03 04 00 0d 00 20 00 1e 04 03 05 03 06 03 02 03 08 04 08 05 08 06 04 01 05 01 06 01 02 01 04 02 05 02 06 02 0 2 02 00 2d 00 02 01 01 00 1c 00 02 40 01 00 15 00 57 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 0 0 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 0 0 00 00 00 00 00 29 00 일 00 b8 00 b2 2c 03 5d 82 93 59 ee 5f f7 af 4e c9 00 00 00 00 26 2a 64 94 dc 48 6d 2c 8a 34 cb 33 fa 90 bf 1b 00 70 광고 3c 49 88 83 c9 36 7c 0 9 a2 be 78 5a 기원전 55 cd 22 60 97 a3 a9 82 11 72 83 f8 2a 03 a1 43 ef d3 ff 5d d3 6d 64 e8 61 be 7f d6 1d 28 27 db 27 9c ce 14 50 77 d4 54 a3 66 4d 4e 6d a4 d2 9e e0 37 25 a6 a4 다 fc d0 fc 67 d2 애 a7 05 29 51 3e 3d a2 67 7f a5 90 6c 5b 3f 7d 8f 92 f2 28 bd a4 0d 다 72 14 70 f9 fb f2 97 b5 애 a6 17 64 6f ac 5c 03 27 2e 97 07 27 c6 21 a7 91 41 ef 5f 7d e6 50 5e 5b fb c3 88 e9 33 43 69 40 93 93 4a e4 d3 57 fa d6 aa cb
...
- 01 04 02 05 02 06 02 02 02 00 2d 00 02 01 01 00 1c 00 02 40 01 00 15 00 57 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 0 0 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 29 00 dd 00 b8 00 b2 2c 03 5d 82 93 59 ee 5f f7 af 4e c9 00 00 00 00 26 2a 64 94 dc 48 6 d 2c 8a 34 cb 33 파 90 bf 1b 00 70 광고 3c 49 88 83 c9 36 7c 09 a2 be 78 5a 기원전 55 cd 22 60 97 a3 a9 82 11 72 83 f8 2a 03 a1 43 ef d3 ff 5d d3 6d 64 e8 61 be 7f d6 1 d 28 27 db 27 9c ce 14 50 77 d4 54 a3 66 4d 4e 6d a4 d2 9e e0 37 25 a6 a4 da fc d0 fc 67 d2 ae a7 05 29 51 3e 3d a2 67 7f a5 90 6c 5b 3f 7d 8f 92 f2 28bd a4 0d 다 72 14 70 f9 fb f2 97 b5 ae a6 17 64 6f ac 5c 03 27 2e 97 07 27 c6 21 a7 91 41 ef 5f 7d e6 50 5e 5b fb c3 88 e9 33 43 69 40 93 93 4a e 4d3 57파 d6 AA cb 00 21 20 3a dd 4f b2 d8 fd f8 22 a0 캘리포니아 3c f7 67 8e f5 e8 8d 애 99 01 41 c5 92 4d 57 bb 6f a3 1b 9e 5f 9d
...
{client}는 초기 애플리케이션 데이터에 대한 쓰기 트래픽 키를 파생합니다.
...
{client}는 EndOfEarlyData 핸드셰이크 메시지를 구성합니다.
...
{client}는 핸드셰이크 데이터에 대한 쓰기 트래픽 키를 파생합니다.
...
이 예에서 클라이언트는 X25519 \[RFC7748\] 공유와 핸드셰이크를 시작합니다. 그러나 서버는 P-256\[FIPS.186-4.2013\]을 선호하고 클라이언트가 P-256 곡선에서 키 공유를 생성하도록 요구하는 HelloRetryRequest를 보냅니다.

참고: HelloRetryRequest는 ServerHello와 동일한 핸드셰이크 메시지 유형을 사용하므로 여기에서 ServerHello로 레이블이 지정됩니다.
...
- 전체 레코드\(181 옥텟\): 16 03 03 00 b0 02 00 00 ac 03 03 cf 21 ad 74 e5 9a 61 11 be 1d 8c 02 1e 65 b8 91 c2 a2 11 16 7a bb 8c 5e 07 9e 09 e2 c8 a8 3 3 9c 00 13 01 00 00 84 00 33 00 02 00 17 00 2c 00 74 00 72 71 dc d0 4b b8 8b c3 18 91 19 39 8a 00 00 00 00 에파 fc 76 c1 46 b8 23 b0 9 6 f8 aa ca d3 65 dd 00 30 95 3f 4e df 62 56 36 e5 f2 1b b2 e2 3f CC 65 4b 1b 5b 40 31 8d 10 d1 37 ab cb b8 75 74 e3 6e 8a 1f 02 5f 7d fa 5d 6e 50 78 1b 5e 다 4a a1 5b 0c 8b e7 78 25 7d 16 aa 30 30 e9 e7 84 1d d9 e4 c0 34 22 67 e8 ca 0c af 57 1f b2 b7 cf f0 f9 34 b0 00 2b 00 02 03 04

{client}는 임시 P-256 키 쌍을 생성합니다.
...
- 전체 레코드\(667 옥텟\): 17 03 03 02 96 99 be e2 0b af 5b 7f c7 27 bf ab 62 23 92 8a 38 1e 6d 0c f9 c4 da 65 3f 9d 2a 7b 23 f7 de 11 cc e8 42 d5 cf 75 63 17 63 45 0f fb 8b 0c c1 d2 38 e6 58 af 7a 12 ad c8 62 43 11 4a b1 4a 1d a2 fa e4 26 21 ce 48 3f b6 24 2e ab fa ad 52 56 6b 02 b3 1d 2e dd ed 에프 엡 80 e6 6a 99 00 d5 f9 73 b4 0c 4f df 74 71 9e cf 1b 68 d7 f9 c3 b6 ce b9 03 ca ​​13 dd 1b b8 f8 18 7a e3 34 17 e1 d1 52 52 2c 58 22 a1 a0 3a d5 2c 83 8c 66 18 97 07 9e 5e a5 00 74 49 e3 19 7b dc 7c 9b ee ed dd ea fd d8 44 af a5 c3 15 ec fe 65 e5 76 af e9 09 81 28 80 62 0e c7 04 8b 42 d7 f5 c7 8d 76 f2 99 d6 d8 25 34 bd d8 f5 12 fe 기원전 0e d3 81 4a 캘리포니아 47 0c d8 00 0d 3e 1c b9 96 2b 05 2f bb 95 0d f6 83 a5 2c 2b a7 7e d3 71 3b 12 29 37 a6 e5 17 09 64 e2 ab 79 69dcd9 80 b3 db 9b 45 8d a7 60 31 24 d6 dc 00 5e 4d 6e 04 b4 d0 c4 ba f3 27 5d b8 27 db ba 0a 6d b0 96 72 17 1f c0 57 b3 85 1d 7e 02 68 41 e2 97 8 f bd 23 46 bb ef dd 03 76 bb 11 08 fe 9a cc 92 18 9f 56 50 aa 5e 85 d8 e8 c7 b6 7a c5 10 db a0 03 d3 d7 e1 63 50 bb 66 d4 50 13 ef d4 4c 9b 60 7c 0d 31 8c 4c 7d 1a 1f 5c BC 57 e2 06 11 80 4e 37 87 d7 b4 a4 b5 f0 8e d8 fd 70 bd ae ad e0 22 60 b1 2a b8 42 ef 69 0b 4a 3e e7 91 1e 84 1b 37 4e cd 5e bb BC 2 54d0 47 b6 00 33 6d d7 d0 c8 8b 4b c1 0e 58 ee 6c b6 56 de 72 47 fa 20 d8 e9 1d eb 84 62 86 08 cf 80 61 5b 62 e9 6c 14 91 c7 ac 37 55 eb 69 01 40 5d 34 74 fe 1a c7 9d 10 6a 0c ee 56 c2 57 7f c8 84 80 f9 6c b6 b8 c6 81 b7 b6 8b 53 c1 46 09 39 08 f3 50 88 81 75 bd fb 0b 1e 31 광고 61 e3 0b a0 광고 fe 6d 2 2 3a a0 3c 07 83 b5 00 1a 57 58 7c 32 8a 9a fc fc fb 97 8d 1c d4 32 8f 7d 9d 60 53 0e 63 0b ef d9 6c 0c 81 6e e2 0b 01 00 76 8a e2 a6 df 5 1 fc 68 f1 72 74 0a 79 후 11 39 8e e3 수 12 52 49 1f a9 c6 93 47 9e 87 7f 94 ab 7c 5f 8c 광고 48 02 03 e6 ab 7b 87 dd 71 e8 a0 72 91 13 df 17 f5 ee e8 6c e1 08 d1 d7 20 07 에크 1c d1 3c 85 a6 c1 49 62 1e 77 b7 d7 8d 80 5a 30 f0 수 03 0c 31 5e 54
...
이 예에서 서버는 클라이언트 인증을 요청합니다. 클라이언트는 RSA 키가 있는 인증서를 사용하고 서버는 P-256 키가 있는 ECDSA\(Elliptic Curve Digital Signature Algorithm\) 인증서를 사용합니다. 이 예에서 사용된 인증서의 개인 키는 표시되지 않습니다.
...
- 키 확장\(16 옥텟\): 6c b6 e6 06 19 d8 c7 35 5c 5d 4c 4b c2 be 90 d5
...
{server}는 CertificateRequest 핸드셰이크 메시지를 구성합니다.
...
- c5 02 79 01 f2 e4 cd 4c a5 b8 07 1e b0 3d 3c 73 2d 83 21 50 66 df c4 d2 91 d4 c1 ff 3b 8d 7e 42 98 f6 77 d4 d5 1d 개 11 68 d8 f1 6c b2 7b a4 02 66 31 3a 1f ed f9 e2 3c c7 7f 76 54 50 f9 e9 6f 05 d0 8f 3d a2 45 b1 4d 49 46 f0 7e c8 1e 6d 56 f2 6b d5 74 f0 b7 f7 c7 04 70 37 c1 6f ce 3b 23 75 4e 66 2f 광고 73 e2 b7 21 3f 6a f2 96 76 9c 99 a1 d3 8e 62 32 e0 ec 8d c4 f8 4d 6a a6 f7 de 38 87 be 00 57 86 2f 90 18 e0 ab 39 67 05 aa 40 90 ab 5 f 2d ff 63 25 a5 57 e7 32 0d 4e ff d4 6b b4 f9 97 d1 63 20 7c ce 66 65 29 4a a4 46 55 41 e3 fe 37 ee 73 50 65 9e a5 50 d6 dc b6 af 3c 51 88 52 c7 a1 4 c 3c c1 5b c3 2b 32 73 bd f1 75 1d a1 84 20 31 35 b1 17 d3 00 20 4f b1 2d 58 ca 9a c3 4b 68 ec a2 70 30 83 2f 7a 4b 46 d2 a5 57 57 f6 3f e8 f6 e8 5a c4 74 69 e6 19 8d a8 8a 64 58 6b f2 3c 69 59 0d e8 22 26 3b e7 5f d8 36 84 72 40 c4 8f 8c 14 5c d6 bd 69 89 62 e7 ed c2 34 eb e5 92 31 35 1e ef 8 d 76 52 참조 3b 08 ab 3a f6 e5 ec 74 c5 8a 8d a3 4b 39 f9 b0 d6 c4 27 9a 9a 1f 82 07 17 29 e7 05 9d d7 f7 b9 5b 94 33 c4 68 4c e1 89 1a 6d 33 43 2d 52 에드 db 0b 8c ee 91 81 d4 03 ec CC 12 99 1f 1a d4 aa 62 c3 60 49 71 3a 7b b1 35 fd 다 66 61 a0 5a 93 f8 c1 6f
...
{client}는 CertificateVerify 핸드셰이크 메시지를 구성합니다.
...
- df a1 50 d2 20 50 5d 40 11 b3 4d 09 d5 38 53 eb a6 1a 10 1e 4f 8d ca 47 d8 17 1a 88 4b 19 25 9a 3d d4 8c 5a c1 41 98 3e dc 77 81 4d 2 5e7 f6 6b 비비 db 90 96 83 92 66 e0 65 61 82 8e cf b2 7e af d4 e9 e8 1a 0b 96 e3 bf a4 2d ae 5a d8 03 59 b9 a6 66 14 02 c3 a2 10 41 77 03 01 06 db d8 f6 5b b 6시 15분 9d 51 2e b1 3a f2 2a 25 9f 31 3b d5 8c 2e 21 fe 05 3d 57 f2 a9 62 b0 a4 ea 68 2c 96 f7 0b 79 b5 60 13 61 92 82 3b 27 be 6a 2f b7 b1 c7 51 참조 c0 e3 30 36 15 54 14 85 b7 b3 07 b4 23 33 2c 11 ef a8 0b 72 f9 b8 0a 53 e5 3f 7b b3 8a 3a f4 c5 9f 80 08 바 d0 54 4e 56 14 e6 88 ff 57 기원전 cd 69 35 에프 8 1F 44 7F 42 0c 1c 1b f4 05 88 18 e9 0b f5 dc 71 6c ca e4 25 24 85 6d f8 25 0b cd bd 7a f6 5f 82 dd 53 06 1d 02 4f 6d 2f f5 c1 1e 37 92 a9 a7 0e 0e e2 a3 c2 0a 1b 96 8a c3 91 f8 f9 28 31 13 5d 25 24 2a 다 2f e2 41 c2 65 3e c9 96 33 9d fa 12 df 애 7a 33 73 df 88 b0 7c a2 7a ef 6d c2 66 a2 5f 13 f7 5c 76 03 9c 1f 46 fd 7a 53 ae 63 99 c9 99 f4 b2 ae e1 8e 48 0d 6d 12 bf ae 22 6b bd c9 2a 6a d5 0b 4d 3b ac 7a BC 3b 36 51 eb 5b e5 6f 33 bf 41 12 7b 3 c a8 86dc 71 4a 50 d1 49 03 57 bd 40 d9 fd 6b e4 22 09 a4 dd b9 eb b2 98 7e 29 f1 20 f0 58 14 61 4d 2c 79 32 00 15 b4 61 페 73 24 44 76 70 a1 후 5f 65년경 15년 b4 74 ab 7f aa 49 50 16 광고 f8 08 e5 3b 94 ef 54 af bb 0e 0a 3a 27 32 ab 59 7f 7d 59 23 c7 73 86 aa 51 24 73 1f 8c c7 3e 70 3b 34 1c 17 5a 45 49 39 a7 7a b6 43 13 c1 5c f3 fe 03 c4 f3 38 42 56 49 76
...
이 예는 \[TLS13\]의 부록 D.4에 정의된 대로 서버가 호환성 모드를 사용하도록 요청하는 클라이언트와의 핸드셰이크 사용을 보여줍니다.
...
이 문서에 포함된 개인 키를 사용하는 것은 좋은 생각이 아닐 수 있습니다. 의미 있는 보안을 제공하기에는 너무 작다는 사실 외에도 이제는 매우 잘 알려져 있습니다.
...
이 문서는 NSS\(네트워크 보안 서비스\)용으로 작성된 테스트를 사용하여 생성되었습니다. Franziskus Kiefer, Eric Rescorla 및 Tim Taubert가 없었다면 이 중 어느 것도 가능하지 않았을 것입니다. 이들은 모두 NSS에서 많은 작업을 수행했습니다.
